...
export async function researchLookup({q,kind='auto',topic='',context='',local=[],terms=[]},{settings,ai,translate,fetchJSON}){
 ...
 if(s.online&&kind==='word'&&/^[a-z-]+$/i.test(q)){
   try{const rows=await fetchJSON('https://api.dictionaryapi.dev/api/v2/entries/en/'+encodeURIComponent(q),{},8000);const phonetics=rows.flatMap(r=>r.phonetics||[]);result.phonetic=phonetics.find(p=>p.text)?.text||result.phonetic;result.audio=phonetics.filter(p=>/^https:\/\//.test(p.audio||'')).map(p=>({url:p.audio,license:p.license?.name||'',sourceUrl:p.sourceUrl||''})).slice(0,2);
     if(!entry&&!s.aiEnabled){
       const defs=rows.flatMap(r=>(r.meanings||[]).flatMap(m=>(m.definitions||[]).slice(0,2).map(d=>({...d,partOfSpeech:m.partOfSpeech,sourceUrl:r.sourceUrls?.[0]||'',license:r.license?.name||''})))).slice(0,4);
       result.senses=await Promise.all(defs.map(async d=>{let zh='',exampleZh='';try{[zh,exampleZh]=await Promise.all([translate(d.definition),d.example?translate(d.example):Promise.resolve('')]);}catch(e){result.error=e.message;}return {en:d.definition,zh,exampleEn:d.example||'',exampleZh,partOfSpeech:d.partOfSpeech,domain:'通用词典义项',source:`Free Dictionary API / 英文词典 ${d.license}；中文为机器翻译`,sourceUrl:d.sourceUrl};}));
       result.source='Free Dictionary API + MyMemory';result.meaning=result.senses.filter(x=>x.zh).map(x=>x.zh).join('；');
     }
   }catch(e){if(!entry)result.error='在线词典未收录或连接失败；'+e.message;}
 }
 if(s.aiEnabled){
 ...
 }else if(!result.meaning&&s.online){try{result.meaning=await translate(q,{basic:true});result.source='MyMemory 词语翻译';}catch(e){result.error=e.message;}}
